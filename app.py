...
type_filter = st.radio("Show", options=["All", "Serial only", "Parallel only"], horizontal=False)
...
compact = st.toggle("Compact mobile view", value=True)

if compact:
    show_cols = ["Metric","Volume","Volume max (Gy)","Max point dose (Gy)","Endpoint"]
else:
...
if not compact and "Type" in df_show.columns:
